...
export default {
    ...
    */
    ...
    loadImg(key, w, h, defaultIcon = '', isAddWaterMark = true) {
        ...
        defaultIcon = window.export_minas.cdn_host || process.env.CDN + defaultIcon;
    ...
    loadAvatar(key, w, h, defaultIcon = '/statics/img/default_avatar.png', addWater = false) {
        let url = this.loadImgUseCrop(key, w, h, addWater, defaultIcon);
        return url;
    },
    // 加载1e1c缩放的头像
    loadOriginalAvatar(key, w, h, defaultIcon = '/statics/img/default_avatar.png') {
        if (!key) return process.env.CDN + defaultIcon;
        return `${process.env.CDN}/${key}@${h}h_${w}w_1e_1c.png`;
    },
    /**
     * 加载预览的大图，目标大于原图不缩放，小于的时候按长边等比缩放
     */
    loadPreviewImg(key, w, h, addWater = false, defaultIcon) {
        let url = this.loadImgBasic(key, {
            width: w,
            height: h
        }, {
        ...
        return this.done(url, defaultIcon);
    },
    /**
     * 单边加载，固定一边，另一边按比例
     * 空的一边传‘’
     */
    loadForOneSideImg(key, w, h, addWater = false, defaultIcon) {
        let url = this.loadImgBasic(key, {
            width: w,
            height: h
        }, {
    ...
    done(url, defaultIcon) {
        if (url) {
            return url;
        } else {
            return (window.export_minas.cdn_host || process.env.CDN) + defaultIcon;
        }
    }
};