<template>
<div class="mz-datepicker" :style="{width:width}" @click.stop.prevent="handlerEvent">
	<input :value="ShowValue" readonly :disabled="disabled" :class="{focus:show}" @click="clickTimeBtn" @mousedown="$event.preventDefault()"/><a v-if="clearable&&ShowValue" @click.stop="clear"></a><i @click="clickTimeBtn"></i>
     <!-- v-if="show" -->
     <!-- @mousedown="$event.preventDefault()" -->
	<div class="mz-datepicker-popup" v-if="show" :class="{'mz-datepicker-popup-left':left}" transition="mz-datepicker-popup" tabindex="-1"  @keyup.up="changeMonth(-1,1)" @keyup.down="changeMonth(1,1)" @keyup.left="changeYear(-1,1)" @keyup.right="changeYear(1,1)" v-el:popup>
		<div class="mz-calendar-top" v-if="range&&!en">
			<template v-for="item in ranges"><i v-if="$index"></i><a v-text="item.name" @click="selectRange(item)"></a></template>
		</div>
		<div :class="{'mz-calendar-range':range}">
			<template v-for="no in count">
			<div class="mz-calendar">
				<div class="mz-calendar-header">
					<a class="mz-calendar-prev-year" :title="prevYearTitle" @click="changeYear(-1,no+1)">«</a><a class="mz-calendar-prev-month" :title="prevMonthTitle" @click="changeMonth(-1,no+1)">‹</a><a v-if="!en" class="mz-calendar-year-select" :title="selectYearTitle" @click="showYear(no+1)">{{this['now'+(no+1)].getFullYear()+(en?"":"年")}}</a><a v-if="!en" class="mz-calendar-month-select" :title="selectMonthTitle" @click="showMonth(no+1)">{{months[this['now'+(no+1)].getMonth()]}}</a><a v-if="en" class="mz-calendar-month-select" :title="selectMonthTitle" @click="showMonth(no+1)">{{months[this['now'+(no+1)].getMonth()]}}</a><a v-if="en" class="mz-calendar-year-select" :title="selectYearTitle" @click="showYear(no+1)">{{this['now'+(no+1)].getFullYear()+(en?"":"年")}}</a><a class="mz-calendar-next-month" :title="nextMonthTitle" @click="changeMonth(1,no+1)">›</a><a class="mz-calendar-next-year" :title="nextYearTitle" @click="changeYear(1,no+1)">»</a>
				</div>
				<table cellspacing="0" cellpadding="0" v-if="this['date'+(no+1)]">
    				<tr>
    					<th v-for="day in days" v-text="day">
    					</th>
    				</tr>
                    <!--单个时间框-->
    				<tr v-for="i in 6">
    					<td v-for="j in 7" v-text="this['date'+(no+1)][i * 7 + j].text" :title="this['date'+(no+1)][i * 7 + j].title" :class="this['date'+(no+1)][i * 7 + j].status" @click="select(this['date'+(no+1)][i * 7 + j], no+1)">
    					</td>
    				</tr>
				</table>
				...
				<div class="mz-calendar-year-panel" transition="mz-calendar-panel" v-if="this['showYear'+(no+1)]">
					<a class="mz-calendar-year-panel-prev" @click="changeYearRange(no+1,-1)"></a><a class="mz-calendar-year-panel-year" v-for="item in this['years'+(no+1)]" :class="item.status" @click="selectYear(item,no+1)">{{item.year+(en?"":"年")}}</a><a class="mz-calendar-year-panel-next" @click="changeYearRange(no+1,1)"></a>
				</div>
				<div class="mz-calendar-month-panel" transition="mz-calendar-panel" v-if="this['showMonth'+(no+1)]">
					<a v-for="item in this['months'+(no+1)]" class="mz-calendar-month-panel-month" :class="item.status" @click="selectMonth(item,no+1)">{{months[item.month-1].substr(0,3)}}</a>
				</div>
			</div>
			<div class="mz-calendar-separator" v-if="range&&no===0">
				<span>{{toTitle}}</span>
			</div>
			</template>
		</div>
		<div class="mz-calendar-bottom" v-if="range || accurate " @click.stop.prevent>
            <div class="accurate-wrap" v-for="i in count" :class="{ secondtime: i === 1 }">
                <div class="accurate-wrap-h">
                        <div v-if="i===0">
                            <select class="form-accurate-h" v-model="hours1" >
                                <option @click.stop.prevent :value="i" v-for="i in 24">{{ i | showH }}</option>
                            </select>
                            <span class="h-text">
                                时
                            </span>
                    	...
                    	<div v-if="i===1">
                            <select class="form-accurate-h" v-model="hours2">
                                <option :value="i" v-for="i in 24">{{ i | showH }}</option>
                            </select>
                            <span>
                                时
                            </span>
			...
			<a class="mz-calendar-btn ok" @click="ok">{{okTitle}}</a>
		</div>
	</div>
</div>	
</template>
	...
	export default {
        ...
        data: function() {
            return {
                ...
                // hours1: 0,  //第一个小时
                hours2: 1,
                singleTimeSelectTime: null  //单个时间狂点击的时间
            };
        },
        computed: {
            ShowValue: function() {
                if (this.range) {
                    ...
                    return this.stringify(this.parse(this.time, false, true));
                }
            },
            isAccurate: function () {
                return !this.range && !!this.accurate;
            }
        },
        filters: {
            showH ( v ) {
        ...
        ready (){
            if( !this.notime ){ 
                //初始化时间为当前时间    
                this.time = this.time || this.stringify( new Date(), this.format );
            }
        ...
        methods: {
            handlerEvent () {
                this.$dispatch( 'handlerEvent' );
            },
            //转换输入的时间
            parse: function(time, isLast, isAll) {
                if (time) {
                    ...
                    } else if( !isLast && !isAll ){
                        return new Date(tmpTime.getFullYear(), tmpTime.getMonth(), tmpTime.getDate());
                    }else if( !isLast && isAll ){
                        // return time;
                        return new Date(tmpTime.getFullYear(), tmpTime.getMonth(), tmpTime.getDate(), tmpTime.getHours());
                    }
                }
                return null;
            },
            ...
            select: function(item, no) {
                ...
                //单个时间框
                if (!self.range && !this.accurate ) {
                    self.time = self.getOutTime(item.time);
                    self.show = false;
                }else if ( !self.range && this.accurate ) {
                    this.singleTimeSelectTime = self.getOutTime(item.time);
            ...
            ok: function() {
                ...
                }else if ( !this.range && this.accurate ) {  //单个并且有时分选择
                    console.log( 111 );
                    // this.time = 
                    //显得出当前的时间 之后在处理
                    var time = this.singleTimeSelectTime;
                        time = this.stringify( this.parse( time ), 'yyyy-MM-dd' );
                        time = new Date( time ).setHours( this.hours1 );
                        time = new Date( time );
                        ...
                        (function ( time ) {
                            console.log( new Date(time) );
                                ...
                                var result =  format.replace(/y+|M+|d+|H+|h+|m+|s+|S+/g, function(str) {
                                    return map[str];
                                });
                                this.time = new Date( result ).getTime();
                        }).call(this, time );
                    // this.time = this.stringify( time );
                }
            },
            //选择范围
            ...
            update: function(time, no) {
                ...
                let day;
                let arr = [];
                time.setDate(0); //切换到上个月最后一天  设置0 就是上个月的最后一天 1-31才是对的
                curFirstDay = time.getDay(); //星期几
                lastDay = time.getDate(); //上个月的最后一天
                for (i = curFirstDay; i > 0; i--) {
                    day = lastDay - i + 1;
                    if( this.ranges && !this.isAccurate  ) {
                        tmpTime = new Date(time.getFullYear(), time.getMonth(), day );
                    }else if( this.isAccurate ) {
                        tmpTime = new Date(time.getFullYear(), time.getMonth(), day, this.hours1 );
                    }
                    tmpTime = this.parse(tmpTime, no === 2, this.isAccurate);
                    arr.push({
                ...
                for (i = 1; i <= curDay; i++) {
                    if( this.isAccurate ) {
                        tmpTime = new Date(time.getFullYear(), time.getMonth(), i, this.hours1 );
                    }else {
                ...
                for (i = 1; arr.length < 42; i++) {
                    tmpTime = new Date(time.getFullYear(), time.getMonth() + 1, i);
                     if( this.isAccurate ) {
                        tmpTime = new Date(time.getFullYear(), time.getMonth() + 1, i, this.hours1);
                    }else {
                        tmpTime = new Date(time.getFullYear(), time.getMonth() + 1, i);
                    }
                    tmpTime = this.parse(tmpTime, no === 2, this.isAccurate);
...
.accurate-wrap {
    ...
    .accurate-wrap-h {
        ...
        .form-accurate-h {
            width: 30px; outline: 0;  height: 20px; border-radius: 6px; text-align: center; padding-left: 7px; background: #fff; 
            option {
...
.mz-datepicker > input:focus, .mz-datepicker > input.focus {
    border-color: #0074ff;
    box-shadow: 0 0 5px rgba(0, 116, 255, .3);
}
.mz-datepicker > i {
    display: inline-block;
    position: absolute;
