...
THREE.EditorControls = function ( object, domElement ) {
    ...
    this.upDirector = new THREE.Vector3( 0, 0, 1 );
    ...
    var STATE = { NONE: -1, ROTATE: 2, ZOOM: -1, PAN: 1 }
    ...
    var EPS = 0.000001;
    var quarterPI = Math.PI / 6;
    var tQuarterPI = Math.PI / 6 * 5;
    ...
    // compute real-time up of camera
    this.updateUpDirector = function () {

        eye.subVectors( object.position, center ).normalize();
        right.crossVectors( object.up, eye ).normalize();
        this.upDirector.crossVectors( eye, right );
    ...
    this.rotate = function ( delta ) {
        ...
        this.updateUpDirector();

        if ( object.up.x ) {

            delta.multiplyScalar( object.up.x );

            theta = Math.atan2( vector.y, vector.z );
            phi = Math.acos( THREE.Math.clamp( vector.x / radius, - 1, 1 ) );
            ...
            if ( phi < quarterPI || phi > tQuarterPI ) {
            // if ( phi === EPS || phi === Math.PI - EPS ) {
                if (  Math.abs( vector.y ) > Math.abs( vector.z ) ) {
                    object.up.set( 0, Math.sign( this.upDirector.y ), 0 );
                } else {
                    object.up.set( 0, 0, Math.sign( this.upDirector.z ) );
                }
            }
            ...
            delta.multiplyScalar( object.up.y );

            theta = Math.atan2( vector.x, vector.z );
            phi = Math.acos( THREE.Math.clamp( vector.y / radius, - 1, 1 ) );
            ...
            // if ( phi === EPS || phi === Math.PI - EPS ) {
                if (  Math.abs( vector.x ) > Math.abs( vector.z ) ) {
                    object.up.set( Math.sign( this.upDirector.x ), 0, 0 );
                } else {
                    object.up.set( 0, 0, Math.sign( this.upDirector.z ) );
                }
            }
            ...
            delta.multiplyScalar( object.up.z );

            theta = Math.atan2( vector.x, vector.y );
            phi = Math.acos( THREE.Math.clamp( vector.z / radius, - 1, 1 ) );
    ...
    function onMouseDown( event ) {
        ...
        if ( event.button === 0 ) {
            ...
            // state = STATE.ROTATE;
            // remove event on mouse's left btn
            state = STATE.NONE;
            return;
